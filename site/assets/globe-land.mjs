    ...
    if (!vertices.has(k))
      vertices.set(
        ...
        surface(
          ...
          Math.max(
            -Math.PI / 2,
            Math.min(Math.PI / 2, (v * side) / 2 - Math.PI / 2),
          ),
        ),
      );
    return vertices.get(k);
  };
  for (let row = 0; row * dy <= Math.PI; row++)
    for (let col = 0; col < columns; col++) {
      ...
      const corners = [
        ...
        [0, -2],
        [1, -1],
      ].map(([du, dv]) => vertex(u + du, v + dv));
      const insetVertices = corners.map((corner) => {
        const p = corner.map(
          (value, i) => center[i] + (value - center[i]) * cellScale,
        );
