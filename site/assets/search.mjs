...
async function search(){reset();const accession=normalizeAccession(input.value);if(!accession||!/\d{3}$/.test(accession)){showError('Invalid accession. Enter a complete accession, such as DRR815000.');return;}const controller=new AbortController();request=controller;let navigating=false;setSearchBusy(submit,true);try{await loadProfile(accession,controller.signal);if(controller.signal.aborted)return;location.assign('./profile.html?accession='+encodeURIComponent(accession));navigating=true;}catch(e){if(e.name!=='AbortError')showError(e.message.startsWith('Accession not found')?'No matching accession. Check the accession and try again.':e.message);}finally{if(request===controller&&!navigating){setSearchBusy(submit,false);}}}
