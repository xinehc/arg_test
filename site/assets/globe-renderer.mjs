import { pointRadius } from "./chart-utils.mjs?v=code-cleanup-1";
...
export function drawLocationMarkers(
  ...
  markers,
  color,
  { ca, sa, ct, st, cr, sr, cx, cy, r, size, zoom },
) {
  ...
  // Small filled dots leave the land layer visible underneath.
  ctx.fillStyle = color;
  for (const m of markers) {
    if (!m.weight) continue;
    const rx = m.x * ca + m.z * sa,
      rz = m.z * ca - m.x * sa,
      ry = m.y * ct - rz * st,
      depth = m.y * st + rz * ct;
    if (depth <= 0) continue;
    const ux = rx * cr - ry * sr,
      uy = rx * sr + ry * cr;
    const px = cx + ux * r,
      py = cy - uy * r,
      dot = pointRadius(m.weight, size, zoom) * 0.55;
    ctx.globalAlpha = 0.6 * horizonOpacity(depth, 0.24);
    ctx.beginPath();
