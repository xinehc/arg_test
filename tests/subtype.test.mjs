...
import {
  ...
  loadSubtype,
  loadSubtypeCatalog,
} from "../site/assets/subtype-data.mjs";
...
test("duplicate subtype names remain separate type/subtype pairs", () => {
  for (const [name, types] of [
    ["bcrB", ["bacitracin", "biocide"]],
    ["cprA", ["bacteriocin", "colistin"]],
  ]) {
    const matches = findSubtypes(entries, name).filter(
      (entry) => entry.subtype === name,
    );
    ...
    for (const entry of matches)
      assert.deepEqual(findSubtypes(entries, `${entry.type}|${name}`), [entry]);
  }
});
test("search is literal and case-insensitive, while identity is exact", () => {
  assert.ok(
    findSubtypes(entries, " BLATEM ").some(
      (entry) => entry.subtype === "blaTEM",
    ),
  );
  for (const name of ["aac(2')*", "tet(C)"])
    assert.ok(
      findSubtypes(entries, name).some((entry) => entry.subtype === name),
    );
...
test("coordinates include zero and boundaries but reject missing or invalid values", () => {
  ...
  for (const value of [
    ...
    assert.equal(coordinates(value), null);
});
test("all indexed files preserve identities, counts, numeric values, and slash names", async () => {
  assert.deepEqual(
    (await readdir(root)).filter((name) => name.endsWith(".txt.gz")).sort(),
    entries.map((entry) => entry.file).sort(),
  );
  assert.equal(entries.length, 1123);
  assert.equal(
    new Set(entries.map((entry) => pairKey(entry.type, entry.subtype))).size,
    entries.length,
  );
  let total = 0;
  for (const entry of entries) {
    const text = gunzipSync(await readFile(new URL(entry.file, root))).toString(
      "utf8",
    );
    const rows = parseSubtypeFile(text, entry);
    assert.equal(rows.length, entry.shown);
    assert.ok(
      rows.every((row) => row.subtype === `${entry.type}|${entry.subtype}`),
    );
    total += rows.length;
  }
  ...
  for (const name of ["qacA/B", "qacF/L", "blaCMA/CSA", "blaLCR/NPS"]) {
    ...
    assert.ok(entry);
    assert.equal(
      new URL(subtypeUrl(entry), "https://example.org").searchParams.get(
        "subtype",
      ),
      name,
    );
...
test("TSV index rejects unsafe filenames, duplicate identities and invalid counts", () => {
  ...
  for (const file of [
    "../0000.txt.gz",
    "https://other/0000.txt.gz",
    "a/b.txt.gz",
    "a\\b.txt.gz",
  ]) {
    assert.throws(
      () => parseSubtypeIndex(header + row.replace("0000.txt.gz", file)),
      /filename/,
    );
  }
  assert.throws(
    () => parseSubtypeIndex(header + row.replace("\t2\t1\t", "\t0\t1\t")),
    /counts/,
  );
  assert.deepEqual(
    parseSubtypeIndex("\uFEFF" + (header + row).replaceAll("\n", "\r\n")),
    parseSubtypeIndex(header + row),
  );
});
test("selected file must match index identity and row count", async () => {
  const entry = entries[0],
    text = gunzipSync(await readFile(new URL(entry.file, root))).toString(
      "utf8",
    );
  assert.throws(
    () => parseSubtypeFile(text, { ...entry, subtype: "other" }),
    /does not match/,
  );
  assert.throws(
    () => parseSubtypeFile(text, { ...entry, shown: entry.shown - 1 }),
    /row count/,
  );
});
test("search downloads only TSV; lookup downloads only the selected gzip", async () => {
  ...
  globalThis.fetch = async (url) => {
    ...
    requested.push(name);
    return new Response(
      await readFile(new URL(encodeURIComponent(name), root)),
    );
  };
  try {
    ...
    const expected = entries.find(
      (entry) => entry.type === "biocide" && entry.subtype === "qacA/B",
    );
