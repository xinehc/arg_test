...
test('full accession matching prevents suffix, prefix, and cross-prefix collisions',()=>{
 ...
 assert.equal(parseProfile(extractProfile(text,'srr000713'),'SRR000713').total,2);
 ...
 assert.throws(()=>extractProfile(text+block('SRR000713'),'SRR000713'),/Duplicate accession/);
 assert.throws(()=>[...batchProfiles(block('SRR000713').replace('accession\tSRR000713\n',''))],/exactly one/);
...
test('cache stores only the selected profile and distinguishes accessions sharing a batch',async()=>{
 globalThis.document={baseURI:'https://u.github.io/repo/'};const cache=new Map();globalThis.sessionStorage={getItem:k=>cache.get(k),setItem:(k,v)=>cache.set(k,v)};
...
test('new data format supports zero detections and explicitly unavailable abundance',()=>{
 ...
 const p=parseProfile(empty,'SRR000713');assert.equal(p.total,0);assert.equal(p.rows.length,0);
 const unavailable=parseProfile(block('SRR000713','n/a'),'SRR000713');
 assert.equal(unavailable.rows[0].abundance,null);assert.equal(unavailable.total,null);assert.equal(unavailable.typeCount,1);
