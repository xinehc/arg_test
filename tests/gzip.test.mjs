...
const suppliedMetadata=txt.split('[data]')[0].split(/\r?\n/).filter(line=>line.includes('\t')).map(line=>line.split('\t'));
test('uploaded gzip decodes and preserves all metadata fields and seven abundance rows',async()=>{
 ...
 assert.deepEqual(p.metadata,suppliedMetadata);assert.equal(m.sample,'SAMD00008644');assert.equal(m.spot_length,'80');assert.equal(m.published,'2012-02-20 22:01:22');assert.equal(p.rows.length,7);assert.equal(p.typeCount,5);assert.equal(p.rows[5].type,'multidrug@RND');assert.equal(p.rows[5].subtype,'mexI');assert.equal(p.rawText,txt);
});
...
test('loader requests exact gz URL and returns metadata from the same response',async()=>{
 ...
 const old=globalThis.fetch;let calls=0;try{globalThis.fetch=async url=>{calls++;assert.equal(url,'https://user.github.io/repo/data/profiles/713.txt.gz');return new Response(gzipSync(txt));};const p=await loadProfile('drr000713');assert.deepEqual(p.metadata,suppliedMetadata);assert.equal(calls,1);}finally{globalThis.fetch=old;}
});
