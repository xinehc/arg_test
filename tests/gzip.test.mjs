...
import {extractProfile,parseProfile,decodeProfileResponse,loadProfile,profileUrl} from '../site/assets/common.mjs';
...
test('uploaded gzip decodes and preserves all metadata fields and seven abundance rows',async()=>{
 ...
 assert.equal(p.metadata.length,21);assert.equal(m.sample,'SAMD00008644');assert.equal(m.spot_length,'80');assert.equal(m.genome_size,'3924.05');assert.equal(m.published,'2012-02-20 22:01:22');assert.equal(p.rows.length,7);assert.equal(p.typeCount,5);assert.equal(p.rows[5].type,'multidrug@RND');assert.equal(p.rows[5].subtype,'mexI');assert.equal(p.rawText,txt);
});
...
test('header optional; new metadata fields preserved; malformed records rejected',()=>{
 ...
 assert.throws(()=>parseProfile(txt.replace('accession\tDRR000713','accession\tOTHER'),'DRR000713'),/does not match/);
 ...
 assert.throws(()=>parseProfile(txt.replace('[data]','[wrong]'),'DRR000713'),/section/);
});
test('loader requests exact gz URL and returns metadata from the same response',async()=>{
 ...
 const old=globalThis.fetch;let calls=0;try{globalThis.fetch=async url=>{calls++;assert.equal(url,'https://user.github.io/repo/data/profiles/713.txt.gz');return new Response(gzipSync(txt));};const p=await loadProfile('drr000713');assert.equal(p.metadata.length,21);assert.equal(calls,1);}finally{globalThis.fetch=old;}
});
