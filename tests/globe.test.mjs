...
const color = "#427a68";
const marker = (depth, weight = 1) => ({
  x: Math.sqrt(1 - depth * depth),
  y: 0,
  z: depth,
  weight,
});
...
test("land honeycomb shares complete edges and preserves them under rotation", async () => {
  const points = JSON.parse(
    await readFile(
      new URL("../site/assets/globe/land-points.json", import.meta.url),
      "utf8",
    ),
  );
  const cells = buildLandCells(points),
    edges = new Map(),
    ids = new Map();
  let shared = 0;
    ...
    for (let i = 0; i < 6; i++) {
      ...
      if (edges.has(key)) {
        ...
        for (const angle of [0, 0.7, 2.4])
          assert.deepEqual(
            projectLandCell([a, b], view(angle)),
            projectLandCell([prior[1], prior[0]], view(angle)),
          );
      } else edges.set(key, [a, b]);
    }
  }
  assert.ok(
    shared > cells.length * 2,
    "most land edges should meet an adjacent cell",
  );
});

test("land polygons are clipped continuously at the horizon", () => {
  const polygon = projectLandCell(
    [
      [0, 0, 1],
      [1, 0, -1],
      [0, 1, 1],
...
function render(markers, angle = 0) {
  ...
  return {
    alpha: 1 - fills.reduce((remaining, a) => remaining * (1 - a), 1),
    fills,
    ctx,
  };
}
...
test("overlapping samples keep density while crossing former depth buckets", () => {
  const markers = [marker(0.503), marker(0.506)];
  // These nearby surface points share a bucket at angle 0 and separate at .004.
  const buckets = (angle) =>
    markers.map((m) =>
      Math.floor((m.z * Math.cos(angle) - m.x * Math.sin(angle)) * 160),
    );
  assert.equal(new Set(buckets(0)).size, 1);
  assert.equal(new Set(buckets(0.004)).size, 2);
  const before = render(markers),
    after = render(markers, 0.004);
  assert.equal(before.fills.length, 2);
  assert.ok(Math.abs(before.alpha - 0.84) < 1e-12);
...
test("current location file loads every row without biome data", async () => {
  const text = await readFile(
    new URL("../site/assets/globe/locations.tsv", import.meta.url),
    "utf8",
  );
  ...
  assert.ok(
    markers.every((m) => Math.abs(Math.hypot(m.x, m.y, m.z) - 1) < 1e-12),
  );
});
...
test("horizon appearance and former band boundaries have no opacity jump", () => {
  for (const depth of [0, 0.00625, 0.125, 0.2375, 0.24]) {
  ...
  assert.equal(render([marker(-0.1), marker(0.5, 0)]).fills.length, 0);
});
